...
from datasets import RDFTriple
...
class IncrementalDecoder:
    def __init__(
        ...
        use_e2e_double_templates=True,
    ):
        self.fuse_model = fuse_model
        self.sentence_scorer = SentenceScorer(
            device=lms_device, reduce_mode=reduce_mode
        )
        self.export_file_handle = export_file_handle
        self.tokenizer = Tokenizer()
        self.use_e2e_double_templates = use_e2e_double_templates

    def _fill_template(self, template, triple: RDFTriple):
        template = (
            template.replace("<subject>", self.tokenizer.tokenize(triple.subj))
            .replace("<predicate>", self.tokenizer.tokenize(triple.pred))
            .replace("<object>", self.tokenizer.tokenize(triple.obj))
        )
        return template

    def _triple_to_template(self, dataset, triple: RDFTriple) -> str:
        """
        Return lexicalized triple
        :param dataset:
        :param triple:
        :return: str containing the lexicalized sentence
        """
    ...
    def fill_template_double(self, template, sorted_triples):
        ...
        template = (
            template.replace("<subject>", self.tokenizer.tokenize(subj))
    ...
    def _triple_to_template_double(self, dataset, triples):
        ...
        # remaining triples
        additional_triples = [
            triple for i, triple in enumerate(triples) if i != idx1 and i != idx2
        ]
        sentences = []
        ...
        return (
            self.sentence_scorer.select_best(sentences),
            double_triples,
            additional_triples,
        )
    ...
    def _decode_entry(self, dataset, entry):
        ...
        logger.debug(
            triples
        )  # something like [RDFTriple(subj='Atlantic_City_International_Airport',
        # pred='location', obj='Egg_Harbor_Township,_New_Jersey'), RDFTriple(subj='Egg_Harbor_Township,_New_Jersey',
        # pred='country', obj='United_States')]
        ...
        if dataset.name == "e2e" and self.use_e2e_double_templates and len(triples) > 1:
            (
                ...
                used_triples,
                additional_triples,
            ) = self._triple_to_template_double(dataset, triples)

        if not current_text:
            current_text = self._triple_to_template(
                dataset, triples[0]
            )  # lexicalization of the first triple
            used_triples = [triples[0]]
            additional_triples = triples[1:]

        logger.info(f"{current_text}")
        facts += used_triples  # facts list, to store all triples used in current_text

        for step, triple in enumerate(additional_triples):
            logger.info(f"Step #{step+1}")

            template = self._triple_to_template(
                dataset, triple
            )  # this is a plain text (lexicalized triple)
            facts.append(triple)

            beam = self.fuse_model.fuse(current_text, template)
            beam = self._filter_beam(
                beam, facts, dataset
            )  # getting riD of sentences which have lost some of the facts

            if beam:
                logger.info(f"{len(beam)} sentences left in beam")
                current_text = self.sentence_scorer.select_best(
                    beam
                )  # candidate selected based on the LM scorer
            else:
        ...
        if self.export_file_handle:
            self.export_file_handle.write(
                self.tokenizer.detokenize(current_text) + "\n"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, required=True, help="Dataset class.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--bert_base_dir",
        type=str,
        default="lasertagger_tf/bert/cased_L-12_H-768_A-12",
        help="Base directory with the BERT pretrained model",
    )
    parser.add_argument(
        "--max_seq_length", default=128, type=int, help="Maximum sequence length."
    )
    parser.add_argument(
        "--is_uncased",
        default=False,
    ...
    parser.add_argument(
        "--vocab_size", type=str, required=True, help="Phrase vocabulary size."
    )
    parser.add_argument(
        "--reduce_mode", type=str, default="gmean", help="Reduce mode for the LMScorer."
    )
    parser.add_argument("--seed", default=42, type=int, help="Random seed.")
    parser.add_argument(
        "--max_threads", default=8, type=int, help="Maximum number of threads."
    )
    parser.add_argument(
        "--no_export",
        action="store_true",
    ...
    try:
        ...
        logger.error(
            f"Unknown dataset: '{args.dataset}'. Please create a class '{args.dataset}' in 'datasets.py'."
        )
        exit()
    ...
    for split in args.splits:
        ...
        label_map_file = os.path.join(
            args.exp_dir, args.experiment, args.vocab_size, "label_map.txt"
        )
        vocab_file = os.path.join(args.bert_base_dir, "vocab.txt")
        model_path = os.path.join(
            args.exp_dir, args.experiment, args.vocab_size, "models", "export"
        )
