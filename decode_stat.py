...
class IncrementalDecoder:
    ...
    def _triple_to_template(self, dataset, triple):
        """
        Retyurns a sentenceas str (I guess)
        """
    ...
    def _decode_entry(self, dataset, entry):
        ...
        if not current_text:
            current_text = entry.lexs[0]  # this is the sentence obtained by replacing entities in the template for the first triple
            used_triples = [triples[0]]
            additional_triples = triples[1:]
            additional_lexs = entry.lexs[1:]

        logger.info(f"{current_text}")
        facts += used_triples  # facts list, to store all triples used in current_text

        for step, (triple, lexicalization) in enumerate(zip(additional_triples, additional_lexs)):
            logger.info(f"Step #{step+1}")

            template = lexicalization  # again, this is a plain text (lexicalized triple)
            facts.append(triple)

            beam = self.fuse_model.fuse(current_text, template)
            beam = self._filter_beam(
                beam, facts, dataset
            )  # here I get rid of sentences which have lost some of  the facts

            if beam:
                logger.info(f"{len(beam)} sentences left in beam")
                current_text = self.sentence_scorer.select_best(
                    beam
                )  # here the candidate is selected based on the language model scorer
            else:
        ...
        # TODO: uncomment to store to file
